...
/// compositing forward is cheap. Seeking backwards replays from frame 0, which is fast for the same
/// reason: each step is one small blit.
/// </summary>
    ...
    public static string Script => """
        (function () {
          ...
          class ApngPlayer {
            ...
            // Frames are cumulative, so going backwards means replaying. Each step is a small blit
            // because frames carry only their changed rectangle.
            async seek(target) {
              ...
              for (let i = this.index + 1; i <= target; i++) { await this.step(i); }
            ...
            sync() {
              const f = this.apng.frames[this.index] || { dn: 0, dd: 100 };
              let elapsed = 0;
              for (let i = 0; i < this.index; i++) {
                elapsed += this.apng.frames[i].dn / this.apng.frames[i].dd;
              }
            ...
            play() {
              ...
              const tick = async () => {
                ...
                this.timer = setTimeout(async () => {
                  if (!this.playing) { return; }
                  await this.seek(this.index + 1 >= this.total ? 0 : this.index + 1);
                  tick();
                }, ms);
              };
              tick();
            ...
            bind() {
              ...
              this.scrub.addEventListener('input', async () => { this.pause(); await this.seek(+this.scrub.value); });
